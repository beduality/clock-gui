...
class ClockInteractListenerTest {
    ...
    void testRightClickWithClockUS() {
        ...
        // US locale prints AM/PM format (contains narrow/standard spaces depending on JDK, so we assert with startsWith/contains)
        assertTrue(message.startsWith("§6Current Time:§r §b"), "Message was: " + message);
        assertTrue(message.contains("6:00") && (message.contains("AM") || message.contains("a.m.") || message.contains("a. m.")), "Message was: " + message);
    }
    ...
    void testRightClickWithClockNether() {
        ...
        World nether = server.createWorld(new WorldCreator("world_nether").environment(World.Environment.NETHER));
    ...
    void testRightClickWithClockEnd() {
        ...
        World end = server.createWorld(new WorldCreator("world_the_end").environment(World.Environment.THE_END));
    ...
    void testCancelledEvent() {
        ...
        assertNull(player.nextMessage(), "Player should not receive a message if the interact event is cancelled");
    }
    ...
    void testEmptyHand() {
        ...
        assertNull(player.nextMessage(), "Player interacting with an empty hand should not get a message");
    }
    ...
    void testNonClockItem() {
        ...
        assertNull(player.nextMessage(), "Player interacting with a non-clock item should not get a message");
    }
}
